...
:documentation
echo Compiles and runs C/C++ source code files.
echo.
echo Usage:	RUNC [file.cpp ^| file.c ^| /?] [command-line arguments for the C/C++ program]
echo.
echo     No args    Display help. This is the same as typing /?.
echo     -?         Display help. This is the same as not typing any options.
echo     xxx.cpp    This C++ source code file will be compiled (by g++) and run.
echo     xxx.c      This C source code file will be compiled (by gcc) and run.
echo.
echo Command-line arguments for the C/C++ program are acceptable, by following the file name.
echo.
echo This batch program (runc.bat^) is recommended to be set as the default program for C/C++ source code files.
